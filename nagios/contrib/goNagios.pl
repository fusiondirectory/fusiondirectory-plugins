...
my $TS_FILE='/tmp/fusiondirectory_timestamp';
my %Options;
my $nb_user=0;
my $nb_groupe=0;
...
# The connexion parameters are in nagios_fd_bind.conf
my $fd_bind_conf="/etc/fusiondirectory/nagios_fd_bind.conf";
my $fd_ldap_conf="/etc/fusiondirectory/nagios_fd_ldap.conf";
my %config_bind = &read_conf($fd_bind_conf);
my %config = &read_conf($fd_ldap_conf);
...
$msg = MIME::Lite->new(
             ...
             To       => $config{to_email},
             Subject  => "Plugin Nagios FusionDirectory",
             Data     => $stdout
             );
